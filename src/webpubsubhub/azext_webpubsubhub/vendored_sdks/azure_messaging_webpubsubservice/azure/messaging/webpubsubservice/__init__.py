...
def _parse_conn_str(conn_str):
    """AWPS Connection string parser

    :param conn_str: Azure Web PubSub Service connection string.
    """
    ...
    for segment in conn_str.split(";"):
        l = segment.find("=")
        if l >= 0:
            key, value = segment[:l], segment[l+1:]
            params[key.lower()] = value

    if 'endpoint' not in params:
        raise AttributeError("connection string misses 'endpoint' field")
    endpoint = params.pop('endpoint')

    if 'accesskey' not in params:
        raise AttributeError("connection string misses 'accesskey' field")

    if 'version' in params:
        params['api_version'] = params.pop('version')

    from azure.core.credentials import AzureKeyCredential
    return endpoint, AzureKeyCredential(params.pop('accesskey')), params


class WebPubSubServiceClient(object):

    def __init__(self, conn_str="", endpoint="", credential=None, **kwargs):
        # type: (str, str, corecredentials.AzureKeyCredential, Any) -> None
        """Create a new WebPubSubServiceClient instance

        :keyword conn_str: Azure Web PubSub Service connection string
        :type conn_str: ~str
        :keyword endpoint: Endpoint to connect to.
        :type endpoint: ~str
        :keyword credential: Credentials to use to connect to endpoint.
        ...
        if conn_str:
            r = _parse_conn_str(conn_str)
            endpoint = endpoint or r[0]
            credential = credential or r[1]
            r[2].update(kwargs)
            kwargs = r[2]
